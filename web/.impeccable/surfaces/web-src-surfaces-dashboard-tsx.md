...
- **Action:** Cut from main (title-strip field, mirrors `/branches`). Trunk name opens `/branch/main`. Open-merge rows open `/merge/:id`. Branch rows open `/branch/:name`. Overflow is "N more branches →" to `/branches`. Empty merges: one line + To the branches. Empty branches: one line + ghost Cut that focuses the plate.
