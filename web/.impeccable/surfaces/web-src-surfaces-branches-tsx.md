...
- **Action:** Cut from main (title-strip field). Open is the branch name. Delete confirms on the row and names what is lost. An open merge request blocks delete and says why.
- **Proof / content:** Fixture data matching the worked example (`contact-fields` by grace, held by MR 1). `main` is listed as a non-deletable trunk row. Demonstration-data tag on the strip.
